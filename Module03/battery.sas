...
*estimating contrasts without adjustment for multiple comparisons;
PROC GLM; 
  CLASS TYPE; 
  MODEL LPUC = TYPE; 
  ...
  LSMEANS TYPE / CL PDIFF ALPHA=0.01;
;
run;

*adjust for multiple comparisons: Tukey;
PROC GLM; 
  CLASS TYPE; 
  MODEL LPUC = TYPE; 
  ESTIMATE 'DUTY'      TYPE  1  1 -1 -1 / DIVISOR = 2;
...
PROC GLM; 
  ...
  ESTIMATE 'INTERACTN' TYPE  1 -1 -1  1 / DIVISOR = 2;
  CONTRAST 'BRAND'     TYPE  1 -1  1 -1;
  LSMEANS TYPE / ADJUST=BONFERRONI CL PDIFF ALPHA=0.01;
;
run;

*adjust for multiple comparisons: Dunnett;
PROC GLM; 
  CLASS TYPE; 
  MODEL LPUC = TYPE; 
  ESTIMATE 'DUTY'      TYPE  1  1 -1 -1 / DIVISOR = 2;
  ...
  ESTIMATE 'INTERACTN' TYPE  1 -1 -1  1 / DIVISOR = 2;
  CONTRAST 'BRAND'     TYPE  1 -1  1 -1;
  LSMEANS TYPE / ADJUST=DUNNETT CL PDIFF ALPHA=0.01;
;
run;
quit;
